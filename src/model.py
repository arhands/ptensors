...
from feature_encoders import CycleEmbedding, get_edge_encoder, get_node_encoder

# from torch_scatter import scatter_sum
from ptensors.objects1 import TransferData0, TransferData1
from ptensors.ptensors0 import transfer0_0
from ptensors.ptensors1 import (
    linmaps1_1,
    transfer0_1,
    transfer1_0,
    transfer1_1,
...
class SplitLayer0_0(Module):
    ...
    def __init__(self, hidden_channels: int, eps: float, momentum: float) -> None:
        ...
        self.epsilon1 = Parameter(torch.tensor(0.0), requires_grad=True)
        self.epsilon2 = Parameter(torch.tensor(0.0), requires_grad=True)

    def forward(
        self, node_rep: Tensor, edge_rep: Tensor, node2edge: TransferData0
    ) -> tuple[Tensor, Tensor]:
...
class SplitLayer0_1(Module):
    ...
    def __init__(
        ...
        self.epsilon1_1 = Parameter(torch.tensor(0.0), requires_grad=True)
        self.epsilon1_2 = Parameter(torch.tensor(0.0), requires_grad=True)
        self.epsilon2 = Parameter(torch.tensor(0.0), requires_grad=True)
    ...
    def forward(
        self, node_rep: Tensor, edge_rep: Tensor, node2edge: TransferData1
    ) -> tuple[Tensor, Tensor]:
        lift_aggr = transfer0_1(
            node_rep, node2edge, [self.reduce_ptensor, self.reduce_messages]
        )
        ...
        lvl_aggr = transfer1_0(
            lvl_aggr_edge,
            node2edge.reverse(),
            [self.reduce_ptensor, self.reduce_messages, self.reduce_ptensor],
            return_list=True,
        )

        node_out = self.lvl_mlp_2(
            (1 + self.epsilon1_1) * node_rep
            + (1 + self.epsilon1_2) * lvl_aggr[0]
            + lvl_aggr[1]
        )
        edge_out = self.lift_mlp(
            (1 + self.epsilon2) * linmaps1_1(edge_rep, node2edge.target, "mean")
            + lift_aggr
        )
...
class TransferLayer1_1(Module):
    ...
    def __init__(self, hidden_channels: int, eps: float, momentum: float) -> None:
        ...
        self.transfer = AffineTransfer1_1(
            hidden_channels, hidden_channels * _inner_mlp_mult
        )
        self.mlp = Sequential(
            # Linear(hidden_channels,hidden_channels*_inner_mlp_mult,False),
            BatchNorm1d(hidden_channels * _inner_mlp_mult, eps, momentum),
...
class ModelLayer(Module):
    ...
    def forward(
        self, node_rep: Tensor, edge_rep: Tensor, cycle_rep: Tensor, data: PtensObjects
    ) -> tuple[Tensor, Tensor, Tensor]:
        node_out, edge_out_1 = self.node_edge(
            node_rep, edge_rep, data["nodes", "edges", 0]
        )
        edge_out_2, cycle_out = self.edge_cycle(
            edge_rep, cycle_rep, data["edges", "cycles", 1]
        )
...
class Net(Module):
    def __init__(
        ...
        self.layers = ModuleList(
            ModelLayer(
                hidden_dim, dropout, eps, momentum, reduce_ptensors, include_cycle_cycle
            )
            for _ in range(num_layers)
        )
        self.readout = readout
        # finalization layers
        self.pool_mlps = ModuleList(
            [
                Sequential(
    ...
    def forward(self, batch: FancyDataObject, ptens_obj: PtensObjects) -> Tensor:
        ...
        for layer in self.layers:
            node_rep, edge_rep, cycle_rep = layer(
                node_rep, edge_rep, cycle_rep, ptens_obj
            )

        # finalizing model
        reps: list[Tensor] = [
            scatter(
                node_rep, batch.batch, 0, dim_size=batch.num_graphs, reduce=self.readout
            ),
            scatter(
                edge_rep,
                ptens_obj["edges", 0].get_batch0(),
                0,
        ...
        rep: Tensor = torch.sum(
            torch.stack([mlp(rep) for mlp, rep in zip(self.pool_mlps, reps)]), 0
        )
    ...
    @classmethod
    def from_in_memory_ds(
        cls, ds: InMemoryDataset, output_dim: int, args: Namespace
    ) -> Net:
        ...
        if encoder_name == "OGB":
            ...
            node_encoder = lambda: get_node_encoder(
                hidden_channels, "Embedding", num_unique_features
            )
            edge_encoder = get_edge_encoder(
                hidden_channels,
                "Embedding" if ds.num_edge_features == 1 else "EmbeddingBag",
                ds.edge_attr.max().item() + 1,
            )
