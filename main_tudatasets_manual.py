...
def evaluate_dataset(dataset: tu_dataset_type, batch_size: int, lr: int, hidden_channels: int, num_trials: int, seed:int):
    ...
    csv_logger.save()
    scores = []
    loop = tqdm(range(num_trials),position=1,total=num_trials,leave=False)
    ...
    mean = np.mean(scores)
    std = np.std(scores)
    csv_logger.log_metrics({'score_mean':mean,'score_std':std},num_trials)#type: ignore
    if mean > best_mean:
        best_mean = mean
        best_std = std
...
for params in param_sets:
    ...
    evaluate_dataset(**params,seed=0) #type: ignore