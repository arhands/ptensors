...
class Net(torch.nn.Module):
    def __init__(self, hidden_dim: int, num_layers: int, dropout: float, dataset: Literal['ZINC'], residual: bool) -> None:
        ...
        self.inter_message_passing = False

        self.atom_encoder = AtomEncoder(hidden_dim)
        self.clique_encoder = Embedding(4, hidden_dim)
        ...
        for _ in range(num_layers):
            self.bond_encoders.append(BondEncoder(hidden_dim))
            nn = Sequential(
            ...
            self.atom_batch_norms.append(BatchNorm1d(hidden_dim))
        ...
        for _ in range(num_layers):
            ...
            self.clique_batch_norms.append(BatchNorm1d(hidden_dim))
        ...
        self.atom_lin = Linear(hidden_dim, hidden_dim)
        self.clique_lin = Linear(hidden_dim, hidden_dim)
